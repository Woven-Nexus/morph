...
export class RowRenderController implements ReactiveController {
	...
	public get dynamicStyling() {
		...
		return {
			table: {
				[rowHighlightProp]:     'var(--_row-even-background)',
	...
	protected handleTableScroll = (() => {
		...
		const func = () => {
			...
			elements.length = 0;
			elements.push(
				root.elementFromPoint(centerX, top + this.rowHeight * 2)!,
				root.elementFromPoint(centerX, bottom - this.rowHeight * 2)!,
			);
